...
class TsharkProtocol:
    ...
    def __init__(self):
        ...
        self.tsharkProtocolDocType = "tsharkprotocol"
        self.resultSize = Common().getSizeToReturn()

    def importTsharkProtocolData(self, jsonObjects):
        es = Elasticsearch()
        es.indices.create(index=self.esIndex, ignore=400)
        insertedCount = 0
        for json in jsonObjects:
            result = es.index(index=self.esIndex, doc_type=self.tsharkProtocolDocType, body=json)
    ...
    def insertFixedTsharkProtocolData(self, dataId, traffic_all_id, content, className, title, startDate, isDeleted):
        ...
        result = Elasticsearch().update(index=self.esIndex, doc_type=self.tsharkProtocolDocType, body=insertFixed, id = dataId)
    ...
    def updateFixedTsharkProtocolData(self, dataId, traffic_all_id, content, className, title, startDate, isDeleted):
        updateFixed = {"doc": {
            "fixedData": {"traffic_all_id": traffic_all_id, "content": content, "className": className, "title": title,
                          "start": startDate, "isDeleted": isDeleted}}}
        result = Elasticsearch().update(index=self.esIndex, doc_type=self.tsharkProtocolDocType, body=updateFixed, id = dataId)
        return Common().getModfiedCount(result)
    ...
    def deleteAllAnnotationsForTsharkProtocol(self, dataId):
        return Annotations().deleteAllAnnotationsForData(self.tsharkProtocolDocType, dataId)

    # add an annotation to the timeline, not a datapoint
    def addAnnotationToTsharkProtocolTimeline(self, tsharkProtocol, annotationText):
        return Annotations().addAnnotationToTimeline(self.tsharkProtocolDocType, tsharkProtocol, annotationText)
